...
if [ "$(id -u)" = "0" ]; then
    ...
    # Declared-plugin installation is owned by the runtime's Python source
    # provider: molecule_runtime.plugin_sources.install_declared_plugins(). It
    # runs after this script drops to uid 1000 and before adapter setup loads
    # /configs/plugins. The runtime validates the resolved install destination,
    # rejects dot segments, and acquires repository credentials only when an
    # anonymous clone needs them. Keep source parsing, network access, and file
    # copies out of this privileged entrypoint. The template's runtime version
    # floor is the fail-closed half of this cutover; do not reintroduce a shell
    # fork.
